...
public class ObservableKeyedCollection<TKey, TItem> : KeyedCollection<TKey, TItem>, INotifyCollectionChanged, INotifyPropertyChanged where TKey : notnull
{
    ...
    public ObservableKeyedCollection(Func<TItem, TKey>? getKeyForItemDelegate = null, string? keyPropName = null)
    {
        ...
        _keyPropertyName = keyPropName;
        _getKeyForItemDelegate = getKeyForItemDelegate != null ? getKeyForItemDelegate : GetKeyForItemDefault;
            ...
            typeof(TItem).GetInterface(nameof(INotifyPropertyChanging)) != null)
        {
            ...
            {
                if (e.PropertyName != _keyPropertyName) return;
                TItem? item = (TItem?)o;
                if (item != null)
                {
    ...
    private TKey GetKeyForItemDefault(TItem item)
    {
        if (_keyPropertyName != null && item != null)
        {
            PropertyInfo? itemInfo = item.GetType().GetProperty(_keyPropertyName);

            if (itemInfo != null)
            {
                TKey? key = (TKey?)itemInfo.GetValue(item);
    ...
    protected override TKey GetKeyForItem(TItem item)
    {
        ...
        { 
            ...
            if (itemInfo != null)
            {
                key = (TKey?)itemInfo.GetValue(item);
    ...
    private TKey getKeyForItemDefault(TItem item)
    {
        ...
        throw new InvalidOperationException("Unable to get key for item.");
    }

    private void UpdatePropChangeHandlers(TItem item, bool addOrRemove)
    {
        if (item == null || _keyPropertyName == null || ChildPropertyChanging == null || ChildPropertyChanged == null) return;
    ...
    {
        if (_keyPropertyName == null || ChildPropertyChanging == null || ChildPropertyChanged == null) return;
